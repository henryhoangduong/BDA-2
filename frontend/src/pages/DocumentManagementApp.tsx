...
const DocumentManagementApp: React.FC = () => {
  ...
  const [selectedDocument, setSelectedDocument] = useState<SimbaDoc | null>(
    null
  );
  ...
  const stats: DocumentStats = {
    ...
    totalQueries: 145,
    itemsIndexed: collections.reduce(
      (acc, collection) =>
        acc +
        collection.documents.filter((doc) => !doc.metadata.is_folder).length,
      0
    ),
  ...
  const handleUpload = async (files: FileList) => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: `${uploadedDocs.length} ${
          uploadedDocs.length === 1 ? "file" : "files"
