...
async def signup(request: SignUpRequest):
    ...
    try:
        user = await AuthService.sign_up(
            email=request.email, password=request.password, user_metadata=request.metadata
        )
        return user
...
async def signin(request: SignInRequest):
    """Sign in a user"""
    try:
        result = await AuthService.sign_in(email=request.email, password=request.password)
