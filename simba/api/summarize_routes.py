...
class SummarizeDocumentRequest(BaseModel):
    ...
    prioritize: bool = Field(
        default=False, description="Set to true to prioritize this summary task."
    )
...
async def trigger_document_summary(request: SummarizeDocumentRequest):
    ...
    try:
        logger.info(
            f"Received summary request for document_id: {request.document_id}, prioritize: {request.prioritize}"
        )
        ...
        if request.prioritize:
            ...
            logger.info(
                f"Dispatched HIGH PRIORITY summary task for document_id: {request.document_id}, task_id: {task.id}"
            )
        else:
            ...
            task = generate_summary_task.apply_async(
                args=[simbadoc_data], priority=4
            )  # Assuming 4 is a standard/lower priority
            logger.info(
                f"Dispatched REGULAR PRIORITY summary task for document_id: {request.document_id}, task_id: {task.id}"
            )
        ...
        logger.error(
            f"Error processing summary request for document_id {request.document_id}: {e}",
            exc_info=True,
        )
