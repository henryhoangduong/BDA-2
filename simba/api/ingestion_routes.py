        ...
        for folder_path in folder_paths:
            ...
            async def process_file_path(file_path):
                try:
                    ...
                    file = UploadFile(filename=file_path.name, file=io.BytesIO(content))
            ...
            for i in range(0, len(files_to_process), chunk_size):
                ...
                results = await asyncio.gather(
                    *[process_file_path(file_path) for file_path in chunk]
                )
                ...
                # Log progress
                logger.info(
                    f"Processed {i + len(chunk)} of {len(files_to_process)} files from {folder_path}"
                )
