...
def mock_auth_service():
    ...
    with patch.object(AuthService, "sign_up", new_callable=AsyncMock) as mock_sign_up, patch.object(
        AuthService, "sign_in", new_callable=AsyncMock
    ) as mock_sign_in, patch.object(
        AuthService, "sign_out", new_callable=AsyncMock
    ) as mock_sign_out, patch.object(
        AuthService, "reset_password", new_callable=AsyncMock
    ) as mock_reset_password, patch.object(
        AuthService, "refresh_token", new_callable=AsyncMock
...
class TestAuthRoutes:
    ...
    def test_signup_success(self, client, mock_auth_service):
        ...
        mock_auth_service["sign_up"].assert_called_once_with(
            email="new@example.com", password="securePassword123", user_metadata={"role": "user"}
        )
    ...
    def test_signup_error(self, client, mock_auth_service):
        ...
        mock_auth_service["sign_up"].side_effect = ValueError(
            "Failed to sign up: Email already registered"
        )

        # Send a request to the signup endpoint
        response = client.post(
            "/auth/signup", json={"email": "existing@example.com", "password": "securePassword123"}
        )
    ...
    def test_signin_success(self, client, mock_auth_service):
        ...
        response = client.post(
            "/auth/signin", json={"email": "user@example.com", "password": "correctPassword123"}
        )
        ...
        mock_auth_service["sign_in"].assert_called_once_with(
            email="user@example.com", password="correctPassword123"
        )
    ...
    def test_signin_error(self, client, mock_auth_service):
        ...
        mock_auth_service["sign_in"].side_effect = ValueError(
            "Failed to sign in: Invalid credentials"
        )

        # Send a request to the signin endpoint
        response = client.post(
            "/auth/signin", json={"email": "user@example.com", "password": "wrongPassword"}
        )
    ...
    def test_reset_password_success(self, client, mock_auth_service):
        ...
        # Verify the service was called with correct parameters
        mock_auth_service["reset_password"].assert_called_once_with(email="user@example.com")
