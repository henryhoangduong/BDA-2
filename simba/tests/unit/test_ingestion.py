...
def mock_postgres_initialization():
    with patch("simba.database.postgres.PostgresDB._initialize_sqlalchemy"), patch(
        "simba.database.postgres.PostgresDB._get_pool"
    ), patch("sqlalchemy.create_engine"), patch("sqlalchemy.orm.sessionmaker"):
        yield
...
def test_document():
    doc = Document(
        page_content="Test content", metadata={"filename": "test_file.md", "source": "test_file.md"}
    )
    return doc
...
async def test_document_ingestion():
    ...
    test_doc = Document(
        page_content="Test content", metadata={"source": "test_file.md", "filename": "test_doc.md"}
    )
    ...
    simba_doc = SimbaDoc(id=str(uuid.uuid4()), documents=[test_doc], metadata=metadata)
    ...
    with patch("simba.models.simbadoc.SimbaDoc.from_documents", return_value=simba_doc):
        ...
        with patch(
            "simba.core.factories.database_factory.get_database", return_value=mock_db
        ), patch(
            "simba.core.factories.vector_store_factory.VectorStoreFactory.get_vector_store",
            return_value=mock_vector_store,
        ), patch(
            "simba.core.factories.storage_factory.StorageFactory.get_storage_provider",
            return_value=mock_storage,
        ), patch(
            "simba.ingestion.document_ingestion.Loader", return_value=mock_loader
        ), patch(
            "simba.ingestion.document_ingestion.Splitter", return_value=mock_splitter
        ), patch(
            "simba.tasks.generate_summary.generate_summary_task"
        ), patch(
...
def test_ingest_document_task():
    ...
    with patch("simba.tasks.ingestion_tasks.get_database") as mock_db_func, patch(
        "simba.ingestion.document_ingestion.DocumentIngestionService"
    ) as mock_service_class, patch(
        "simba.tasks.ingestion_tasks.ingest_document_task.delay"
    ) as mock_delay:
