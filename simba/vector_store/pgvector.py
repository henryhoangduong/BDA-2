...
class ChunkEmbedding(Base):
    ...
    def to_langchain_doc(self) -> Document:
        ...
        return Document(page_content=self.data["page_content"], metadata=self.data["metadata"])
...
class PGVectorStore(VectorStore):
    ...
    def embeddings(self) -> Optional[Embeddings]:
        ...
        logger.debug(
            f"The embeddings property has not been " f"implemented for {self.__class__.__name__}"
        )
    ...
    def _retrieve_with_dense_vector(
        self, query: str, user_id: str, top_k: int, document_ids: Optional[List[str]] = None
    ) -> List[Document]:
        """
    ...
    def similarity_search(
        ...
        if use_bm25_first_pass and sparse_results:
            fused_results = self._fuse_results_rrf(sparse_results, dense_results, k=60, top_k=top_k)
        else:
    ...
    def get_documents(self, document_ids: List[str], user_id: str) -> List[Document]:
        ...
        try:
            ...
            chunks = (
                ...
                .filter(
                    ChunkEmbedding.document_id.in_(document_ids), ChunkEmbedding.user_id == user_id
                )
                .all()
            )
    ...
    def update_document(
        self, document_id: str, new_document: Document, user_id: str = None
    ) -> bool:
        ...
        try:
            ...
            query = session.query(ChunkEmbedding).filter(ChunkEmbedding.document_id == document_id)
    ...
    def rerank_results(
        ...
        try:
            ...
            logger.warning(
                "Could not import sentence_transformers. Reranking skipped. Install with 'pip install sentence-transformers'"
            )
    ...
    def context_compression(
        self, query: str, documents: List[Document], num_passages: int = 10
    ) -> List[Document]:
        """
