...
class SQLOrganization(Base):
    ...
    members = relationship(
        "SQLOrganizationMember", back_populates="organization", cascade="all, delete-orphan"
    )
...
class OrganizationService:
    ...
    async def create_organization(
        self, organization: OrganizationCreate, user_id: str
    ) -> Organization:
        try:
            session = self._get_session()
            try:
                ...
                user_email = session.execute(
                    text("SELECT email FROM auth.users WHERE id = :user_id"), {"user_id": user_id}
                ).scalar()
                ...
                # Create organization
                org_id = str(uuid4())
                new_org = SQLOrganization(id=org_id, name=organization.name, created_by=user_id)
                session.add(new_org)
    ...
    async def get_organization(self, org_id: str, user_id: str) -> Organization:
        try:
            ...
            try:
                ...
                member = (
                    session.query(SQLOrganizationMember)
                    .filter(
                ...
                org = session.query(SQLOrganization).filter(SQLOrganization.id == org_id).first()
    ...
    async def invite_member(
        self, org_id: str, invite: OrganizationMemberInvite, user_id: str
    ) -> OrganizationMember:
        try:
            session = self._get_session()
            try:
                ...
                member_id = str(uuid4())
                new_member = SQLOrganizationMember(
                    id=member_id, organization_id=org_id, email=invite.email, role=invite.role
                )
                session.add(new_member)
                session.commit()
    ...
    async def update_member_role(
        self, org_id: str, member_id: str, update: OrganizationMemberUpdate, user_id: str
    ) -> OrganizationMember:
        try:
            session = self._get_session()
            try:
